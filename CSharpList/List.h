...
public:
    ...
    _NODISCARD _CONSTEXPR20 reference operator*() const noexcept {
        ...
        _STL_VERIFY(_Ptr, "can't dereference value-initialized List iterator");
        _STL_VERIFY(_Mycont, "can't dereference invalidated List iterator");
        _STL_VERIFY(
            _Mycont->_Myfirst <= _Ptr && _Ptr < _Mycont->_Mylast, "can't dereference out of range List iterator");
...
#if _ITERATOR_DEBUG_LEVEL != 0
        const auto _Mycont = static_cast<const _Myvec*>(this->_Getcont());
        _STL_VERIFY(_Ptr, "can't increment value-initialized List iterator");
        _STL_VERIFY(_Mycont, "can't increment invalidated List iterator");
        _STL_VERIFY(_Ptr < _Mycont->_Mylast, "can't increment List iterator past end");
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
#if _ITERATOR_DEBUG_LEVEL != 0
        const auto _Mycont = static_cast<const _Myvec*>(this->_Getcont());
        _STL_VERIFY(_Ptr, "can't decrement value-initialized List iterator");
        _STL_VERIFY(_Mycont, "can't decrement invalidated List iterator");
        _STL_VERIFY(_Mycont->_Myfirst < _Ptr, "can't decrement List iterator before begin");
#endif // _ITERATOR_DEBUG_LEVEL != 0
    ...
    _CONSTEXPR20 void _Verify_offset(const difference_type _Off) const noexcept {
        ...
        const auto _Mycont = static_cast<const _Myvec*>(this->_Getcont());
        _STL_VERIFY(_Off == 0 || _Ptr, "cannot seek value-initialized List iterator");
        _STL_VERIFY(_Off == 0 || _Mycont, "cannot seek invalidated List iterator");
        if (_Off < 0) {
            _STL_VERIFY(_Off >= _Mycont->_Myfirst - _Ptr, "cannot seek List iterator before begin");
        }

        if (_Off > 0) {
            _STL_VERIFY(_Off <= _Mycont->_Mylast - _Ptr, "cannot seek List iterator after end");
        }
#endif // _ITERATOR_DEBUG_LEVEL == 0
    }
    ...
    friend _CONSTEXPR20 void _Verify_range(
        ...
        _STL_VERIFY(_First._Ptr <= _Last._Ptr, "List iterator range transposed");
    }
#endif // _ITERATOR_DEBUG_LEVEL != 0
    ...
    _Tptr _Ptr; // pointer to element in List
};
    ...
    _NODISCARD static constexpr element_type* to_address(const pointer _Iter) noexcept {
#if _ITERATOR_DEBUG_LEVEL != 0
        // A value-initialized iterator is in the domain of to_address. An invalidated end iterator for a List with
        ...
        if (_Mycont) {
            _STL_VERIFY(_Mycont->_Myfirst <= _Iter._Ptr && _Iter._Ptr <= _Mycont->_Mylast,
                "can't convert out-of-range List iterator to pointer");
        }
        else {
            _STL_VERIFY(!_Iter._Ptr, "can't convert invalid List iterator to pointer");
        }
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
#if _ITERATOR_DEBUG_LEVEL != 0
        const auto _Mycont = static_cast<const _Myvec*>(this->_Getcont());
        _STL_VERIFY(this->_Ptr, "can't dereference value-initialized List iterator");
        _STL_VERIFY(_Mycont, "can't dereference invalidated List iterator");
        _STL_VERIFY(_Mycont->_Myfirst <= this->_Ptr && this->_Ptr < _Mycont->_Mylast,
            "can't dereference out of range List iterator");
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
class List { // varying size array of values
private:
    template <class>
    friend class _Vb_val;
    friend _Tidy_guard<List>;
...
private:
    ...
    _CONSTEXPR20 void _Modify_annotation(const difference_type _Count) const noexcept {
        ...
        if (_Count == 0) { // nothing to do
            ...
            // when the List has zero capacity, see GH-2464.
            return;
        }
    ...
    static _CONSTEXPR20 void _Apply_annotation(
        ...
        if constexpr ((_Container_allocation_minimum_asan_alignment<List>) >= _Asan_granularity) {
            // old state:
    ...
    public:
        ...
        constexpr explicit _Asan_extend_guard(List* _Myvec_, size_type _Target_size_) noexcept
        ...
        _CONSTEXPR20 ~_Asan_extend_guard() {
            ...
            // Shrinks the shadow memory to the current size of the List.
            auto& _My_data = _Myvec->_Mypair._Myval2;
    ...
    public:
        ...
        constexpr explicit _Asan_create_guard(const List& _Myvec_) noexcept : _Myvec(_Myvec_) {}
...
public:
    ...
    _CONSTEXPR20 List() noexcept(is_nothrow_default_constructible_v<_Alty>) : _Mypair(_Zero_then_variadic_args_t{}) {
        _Mypair._Myval2._Alloc_proxy(_GET_PROXY_ALLOCATOR(_Alty, _Getal()));
    }

    _CONSTEXPR20 explicit List(const _Alloc& _Al) noexcept : _Mypair(_One_then_variadic_args_t{}, _Al) {
        _Mypair._Myval2._Alloc_proxy(_GET_PROXY_ALLOCATOR(_Alty, _Getal()));
    }

    _CONSTEXPR20 explicit List(_CRT_GUARDOVERFLOW const size_type _Count, const _Alloc& _Al = _Alloc())
        : _Mypair(_One_then_variadic_args_t{}, _Al) {
        _Construct_n(_Count);
    }

    _CONSTEXPR20 List(_CRT_GUARDOVERFLOW const size_type _Count, const _Ty& _Val, const _Alloc& _Al = _Alloc())
        : _Mypair(_One_then_variadic_args_t{}, _Al) {
        _Construct_n(_Count, _Val);
    }

    template <class _Iter, enable_if_t<_Is_iterator_v<_Iter>, int> = 0>
    _CONSTEXPR20 List(_Iter _First, _Iter _Last, const _Alloc& _Al = _Alloc())
        : _Mypair(_One_then_variadic_args_t{}, _Al) {
    ...
    _CONSTEXPR20 List(initializer_list<_Ty> _Ilist, const _Alloc& _Al = _Alloc())
        : _Mypair(_One_then_variadic_args_t{}, _Al) {
    ...
    constexpr List(from_range_t, _Rng&& _Range, const _Alloc& _Al = _Alloc())
        : _Mypair(_One_then_variadic_args_t{}, _Al) {
    ...
    _CONSTEXPR20 List(const List& _Right)
    ...
    _CONSTEXPR20 List(const List& _Right, const _Identity_t<_Alloc>& _Al)
        : _Mypair(_One_then_variadic_args_t{}, _Al) {
        const auto& _Right_data = _Right._Mypair._Myval2;
        const auto _Count = static_cast<size_type>(_Right_data._Mylast - _Right_data._Myfirst);
        _Construct_n(_Count, _Right_data._Myfirst, _Right_data._Mylast);
    ...
    _CONSTEXPR20 List(List&& _Right) noexcept
    ...
    _CONSTEXPR20 List(List&& _Right, const _Identity_t<_Alloc>& _Al_) noexcept(
    ...
    _CONSTEXPR20 List& operator=(List&& _Right) noexcept(
...
private:
    ...
    _CONSTEXPR20 void _Append_uncounted_range(_Iter _First, const _Sent _Last) {
        ...
        // Otherwise, provide basic guarantee (despite N4928 [List.modifiers]/2).
...
public:
    ...
    _CONSTEXPR20 iterator emplace(const_iterator _Where, _Valty&&... _Val) {
        ...
        _STL_VERIFY(
            ...
            "List emplace iterator outside range");
...
public:
    ...
    constexpr iterator insert_range(const_iterator _Where, _Rng&& _Range) {
        ...
        _STL_VERIFY(
            _Where._Getcont() == _STD addressof(_My_data) && _Whereptr >= _Oldfirst && _My_data._Mylast >= _Whereptr,
            "List insert_range iterator out-of-range");
#endif // _ITERATOR_DEBUG_LEVEL == 2
        const auto _Whereoff = static_cast<size_type>(_Whereptr - _Oldfirst);
    ...
    _CONSTEXPR20 iterator insert(const_iterator _Where, initializer_list<_Ty> _Ilist) {
        ...
        _STL_VERIFY(
            _Where._Getcont() == _STD addressof(_My_data) && _Whereptr >= _Oldfirst && _My_data._Mylast >= _Whereptr,
            "List insert iterator out-of-range");
#endif // _ITERATOR_DEBUG_LEVEL == 2
        const auto _Whereoff = static_cast<size_type>(_Whereptr - _Oldfirst);
...
public:
    ...
    _CONSTEXPR20 List& operator=(const List& _Right) {
        if (this == _STD addressof(_Right)) {
            return *this;
        }
    ...
    _CONSTEXPR20 List& operator=(initializer_list<_Ty> _Ilist) {
        const auto _Count = _Convert_size<size_type>(_Ilist.size());
        _Assign_counted_range(_Ilist.begin(), _Count);
        return *this;
    }
...
public:
...
#if _ITERATOR_DEBUG_LEVEL == 2
        _STL_VERIFY(_My_data._Myfirst != _Mylast, "List empty before pop");
#endif // _ITERATOR_DEBUG_LEVEL == 2
    ...
    _CONSTEXPR20 void clear() noexcept { // erase all
        ...
        if (_Myfirst == _Mylast) { // already empty, nothing to do
            ...
            // Note that when clearing an empty List, this will preserve past-the-end iterators, which is allowed by
    ...
    _CONSTEXPR20 void swap(List& _Right) noexcept /* strengthened */ {
        if (this != _STD addressof(_Right)) {
    ...
    _NODISCARD _CONSTEXPR20 _Ty& operator[](const size_type _Pos) noexcept /* strengthened */ {
        ...
        _STL_VERIFY(
            _Pos < static_cast<size_type>(_My_data._Mylast - _My_data._Myfirst), "List subscript out of range");
    ...
    _NODISCARD _CONSTEXPR20 _Ty& front() noexcept /* strengthened */ {
        auto& _My_data = _Mypair._Myval2;
#if _CONTAINER_DEBUG_LEVEL > 0
        _STL_VERIFY(_My_data._Myfirst != _My_data._Mylast, "front() called on empty List");
#endif // _CONTAINER_DEBUG_LEVEL > 0
    ...
    _NODISCARD _CONSTEXPR20 _Ty& back() noexcept /* strengthened */ {
        auto& _My_data = _Mypair._Myval2;
#if _CONTAINER_DEBUG_LEVEL > 0
        _STL_VERIFY(_My_data._Myfirst != _My_data._Mylast, "back() called on empty List");
#endif // _CONTAINER_DEBUG_LEVEL > 0
...
private:
    ...
    _CONSTEXPR20 void _Construct_n(_CRT_GUARDOVERFLOW const size_type _Count, _Valty&&... _Val) {
        ...
        // 1-arg -> value-construction, e.g. List(5)
        // 2-arg -> fill, e.g. List(5, "meow")
        // 3-arg -> sized range construction, e.g. List{"Hello", "Fluffy", "World"}
        auto& _Al = _Getal();
        auto&& _Alproxy = _GET_PROXY_ALLOCATOR(_Alty, _Al);
        auto& _My_data = _Mypair._Myval2;
        _Container_proxy_ptr<_Alty> _Proxy(_Alproxy, _My_data);
    ...
    _CONSTEXPR20 void _Move_assign_unequal_alloc(List& _Right) {
        auto& _Al = _Getal();
        auto& _My_data = _Mypair._Myval2;
        auto& _Right_data = _Right._Mypair._Myval2;
    ...
    [[noreturn]] static void _Xrange() {
        _Xout_of_range("invalid List subscript");
    }
...
class List<bool, _Alloc>;

using _Vbase = unsigned int; // word type for List<bool> representation
...
_NODISCARD _CONSTEXPR20 bool operator==(const List<_Ty, _Alloc>& _Left, const List<_Ty, _Alloc>& _Right) {
...
// Optimize List<bool> lexicographical comparisons.
...
// * Appending bits to List<bool> eventually appends words to its underlying storage.
...
// * However, List<bool> packs bits into words as little-endian (least significant bit first).
//   For example, List<bool>{false, true, true, true} stores 0b0000'0000'0000'0000'0000'0000'0000'1110u.
...
_NODISCARD constexpr _Synth_three_way_result<_Ty> operator<=>(
    const List<_Ty, _Alloc>& _Left, const List<_Ty, _Alloc>& _Right) {
    if constexpr (is_same_v<_Ty, bool>) {
        // This optimization works because List<bool> "trims" its underlying storage by zeroing out unused bits.
...
_EXPORT_STD template <class _Ty, class _Alloc>
_CONSTEXPR20 void swap(List<_Ty, _Alloc>& _Left, List<_Ty, _Alloc>& _Right) noexcept /* strengthened */ {
    _Left.swap(_Right);
}

#if _HAS_CXX20
_EXPORT_STD template <class _Ty, class _Alloc, class _Uty>
constexpr typename List<_Ty, _Alloc>::size_type erase(List<_Ty, _Alloc>& _Cont, const _Uty& _Val) {
    return _Erase_remove(_Cont, _Val);
}

_EXPORT_STD template <class _Ty, class _Alloc, class _Pr>
constexpr typename List<_Ty, _Alloc>::size_type erase_if(List<_Ty, _Alloc>& _Cont, _Pr _Pred) {
    return _Erase_remove_if(_Cont, _Pass_fn(_Pred));
}
#endif // _HAS_CXX20

template <class _Alloc0>
struct _Wrap_alloc { // TRANSITION, ABI compat, preserves symbol names of List<bool>::iterator
...
public:
    ...
    using _Mycont = List<bool, _Rebind_alloc_t<_Alvbase, bool>>;
...
public:
    ...
    _CONSTEXPR20 const _Vbase* _Getptr() const noexcept {
        ...
        _STL_VERIFY(_Cont, "cannot dereference value-initialized List<bool> iterator");
        _STL_VERIFY(this->_Total_off(_Cont) <= static_cast<_Difference_type>(_Cont->_Mysize),
            "List<bool> iterator not dereferenceable");
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
public:
    ...
    _CONSTEXPR20 _Vb_const_iterator& operator+=(const difference_type _Off) noexcept {
        ...
        if (_Off != 0) {
            const auto _Cont = static_cast<const _Mycont*>(this->_Getcont());
            _STL_VERIFY(_Cont, "cannot seek value-initialized List<bool> iterator");
            const auto _Start_offset = this->_Total_off(_Cont);
            if (_Off < 0) {
                _STL_VERIFY(-_Start_offset <= _Off, "cannot seek List<bool> iterator before begin");
            }
            else if (0 < _Off) {
                _STL_VERIFY(_Off <= static_cast<_Difference_type>(_Cont->_Mysize - _Start_offset),
                    "cannot seek List<bool> iterator after end");
            }
        }
#endif // _ITERATOR_DEBUG_LEVEL != 0
    ...
    _CONSTEXPR20 void _Compat(const _Vb_const_iterator& _Right) const noexcept {
        ...
        _STL_VERIFY(this->_Getcont() == _Right._Getcont(), "List<bool> iterators incompatible");
    ...
    friend _CONSTEXPR20 void _Verify_range(const _Vb_const_iterator& _First, const _Vb_const_iterator& _Last) noexcept {
        ...
        _STL_VERIFY(_First <= _Last, "List<bool> iterator range transposed");
    }
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
#if _ITERATOR_DEBUG_LEVEL != 0
        const auto _Cont = static_cast<const _Mycont*>(this->_Getcont());
        _STL_VERIFY(_Cont, "cannot decrement value-initialized List<bool> iterator");
        _STL_VERIFY(this->_Total_off(_Cont) > 0, "cannot decrement List<bool> begin iterator");
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
#if _ITERATOR_DEBUG_LEVEL != 0
        const auto _Cont = static_cast<const _Mycont*>(this->_Getcont());
        _STL_VERIFY(_Cont, "cannot increment value-initialized List<bool> iterator");
        _STL_VERIFY(this->_Total_off(_Cont) < static_cast<_Difference_type>(_Cont->_Mysize),
            "cannot increment List<bool> end iterator");
#endif // _ITERATOR_DEBUG_LEVEL != 0
...
public:
    ...
    using _Vectype = List<_Vbase, _Alvbase>;
    ...
    _Vectype _Myvec; // base List of words
    size_type _Mysize; // current length of sequence
};

template <class _Alloc>
class List<bool, _Alloc> : public _Vb_val<_Alloc> {
public:
    static_assert(!_ENFORCE_MATCHING_ALLOCATORS || is_same_v<bool, typename _Alloc::value_type>,
        _MISMATCHED_ALLOCATOR_MESSAGE("List<bool, Allocator>", "bool"));
    ...
    _CONSTEXPR20 List() noexcept(is_nothrow_default_constructible_v<_Alloc>) : _Mybase(_Alloc()) {}

    _CONSTEXPR20 explicit List(const _Alloc& _Al) noexcept : _Mybase(_Al) {}

    _CONSTEXPR20 explicit List(_CRT_GUARDOVERFLOW size_type _Count, const _Alloc& _Al = _Alloc())
        : _Mybase(_Count, false, _Al) {
        _Trim(_Count);
    }

    _CONSTEXPR20 List(_CRT_GUARDOVERFLOW size_type _Count, const bool& _Val, const _Alloc& _Al = _Alloc())
        : _Mybase(_Count, _Val, _Al) {
        _Trim(_Count);
    }

    _CONSTEXPR20 List(const List& _Right) : _Mybase(_Right) {}

    _CONSTEXPR20 List(const List& _Right, const _Identity_t<_Alloc>& _Al) : _Mybase(_Right, _Al) {}

    template <class _Iter, enable_if_t<_Is_iterator_v<_Iter>, int> = 0>
    _CONSTEXPR20 List(_Iter _First, _Iter _Last, const _Alloc& _Al = _Alloc()) : _Mybase(_Al) {
        insert(begin(), _First, _Last);
    }

#if _HAS_CXX23 && defined(__cpp_lib_concepts) // TRANSITION, GH-395
    template <_Container_compatible_range<bool> _Rng>
    constexpr List(from_range_t, _Rng&& _Range, const _Alloc& _Al = _Alloc()) : _Mybase(_Al) {
        if constexpr (_RANGES forward_range<_Rng> || _RANGES sized_range<_Rng>) {
            const auto _Length = _To_unsigned_like(_RANGES distance(_Range));
            const auto _Count = _Convert_size<size_type>(_Length);
            _Assign_counted_range(_RANGES _Ubegin(_Range), _Count);
    ...
    _CONSTEXPR20 List(List&& _Right) noexcept : _Mybase(_STD move(_Right)) {
        this->_Swap_proxy_and_iterators(_Right);
    }

    _CONSTEXPR20 List(List&& _Right, const _Identity_t<_Alloc>& _Al) noexcept(
    ...
    _CONSTEXPR20 List(initializer_list<bool> _Ilist, const _Alloc& _Al = allocator_type()) : _Mybase(0, false, _Al) {
        insert(begin(), _Ilist.begin(), _Ilist.end());
    }

    _CONSTEXPR20 List& operator=(initializer_list<bool> _Ilist) {
...
#if _CONTAINER_DEBUG_LEVEL > 0
        _STL_VERIFY(_Off < this->_Mysize, "List<bool> subscript out of range");
#endif // _CONTAINER_DEBUG_LEVEL > 0
...
#if _CONTAINER_DEBUG_LEVEL > 0
        _STL_VERIFY(this->_Mysize != 0, "front() called on empty List<bool>");
#endif // _CONTAINER_DEBUG_LEVEL > 0
...
#if _CONTAINER_DEBUG_LEVEL > 0
        _STL_VERIFY(this->_Mysize != 0, "back() called on empty List<bool>");
#endif // _CONTAINER_DEBUG_LEVEL > 0
...
#if _ITERATOR_DEBUG_LEVEL == 2
        _STL_VERIFY(end() > _Where, "List<bool> erase iterator outside range");
...
#if _ITERATOR_DEBUG_LEVEL == 2
            _STL_VERIFY(_Last >= _First && end() >= _Last, "List<bool> erase iterator outside range");
...
#if _ITERATOR_DEBUG_LEVEL == 2
        _STL_VERIFY(end() >= _Where, "List<bool> insert iterator outside range");
    ...
    _CONSTEXPR20 void _Orphan_range_unlocked(size_type _Offlo, size_type _Offhi) const {
        ...
        while (*_Pnext) { // test offset from beginning of List
    ...
    [[noreturn]] void _Xlen() const {
        _Xlength_error("List<bool> too long");
    }

    [[noreturn]] void _Xran() const {
        _Xout_of_range("invalid List<bool> subscript");
    }
};

template <class _Alloc>
struct hash<List<bool, _Alloc>> {
    using _ARGUMENT_TYPE_NAME _CXX17_DEPRECATE_ADAPTOR_TYPEDEFS = List<bool, _Alloc>;
    using _RESULT_TYPE_NAME _CXX17_DEPRECATE_ADAPTOR_TYPEDEFS = size_t;

    _NODISCARD size_t operator()(const List<bool, _Alloc>& _Keyval) const noexcept {
